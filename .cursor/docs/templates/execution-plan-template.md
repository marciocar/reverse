<!-- AUTO PROMPT PARA [TIPO DE IMPLEMENTAÇÃO] -->
<!-- 
🤖 INSTRUÇÕES PARA [IMPLEMENTAÇÃO/EXECUÇÃO/FINALIZAÇÃO] [NOME DO SISTEMA/COMPONENTE]

ESTADO ATUAL: [DESCRIÇÃO DO ESTADO ATUAL - ex: ARQUITETURA 85% IMPLEMENTADA]
BASE: [TECNOLOGIAS/COMPONENTES JÁ FUNCIONANDO]

COMO USAR ESTE DOCUMENTO:
1. ✅ MARQUE checkboxes [ ] → [x] conforme progresso nas tarefas
2. 📝 ATUALIZE o log de execução a cada sessão de implementação
3. 📅 PREENCHA datas, responsável e observações detalhadas
4. 🔄 EXECUTE comandos listados na seção "Comandos de Execução"
5. 🏷️ CRIE tags git para pontos de rollback de emergência
6. 💰 MONITORE custos [se aplicável] e performance durante implementação
7. 🔄 ATUALIZE as marcações de status de cada fase no documento ao final de cada etapa

PADRÃO DE MARCAÇÃO:
- [x] = Tarefa CONCLUÍDA com sucesso e validada
- [ ] = Tarefa PENDENTE ou não iniciada
- ❌ = Tarefa com ERRO crítico (documentar no log)
- ⚠️ = Tarefa PARCIAL ou com ressalvas importantes
- 🔄 = Tarefa EM PROGRESSO (marcar data de início)

PADRÃO DE COMMITS:
- "feat([módulo]): implementa [funcionalidade principal]"
- "feat([área]): adiciona [componente/sistema específico]"
- "fix([módulo]): corrige [problema específico]"
- "docs([plano]): atualiza documentação fase X"
- "refactor([área]): [tipo de refatoração]"

COMANDOS ESSENCIAIS:
- 📦 SEMPRE usar [gerenciador de pacotes]: [comandos principais]
- [Comando principal]: [descrição]
- [Comando de teste]: [descrição]
- [Comando de validação]: [descrição]
...
MILESTONES CRÍTICOS:
🎯 Fase 1: [Primeiro marco importante]
🎯 Fase 2: [Segundo marco importante]
🎯 Fase 3: [Terceiro marco importante]
🎯 Fase N: [Marco final]

MÉTRICAS DE SUCESSO FINAL:
- 📊 [Métrica 1]: [Target numérico] [critério de medição]
- 🤖 [Métrica 2]: [Target percentual] [critério de medição]
- 🏷️ [Métrica 3]: [Target qualitativo] [critério de medição]
- ⚡ [Métrica 4]: [Target de performance] [critério de medição]
- 🔒 [Métrica 5]: [Target de segurança/qualidade] [critério de medição]
- 🧪 [Métrica 6]: [Target de testes] [critério de medição]

Última atualização automática: [DATA/HORA]
-->
