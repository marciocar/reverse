# SUMÁRIO EXECUTIVO: POLÍTICA DE ANTICORRUPÇÃO

**1 PÁGINA | PARA TODOS OS COLABORADORES**

---

## 🎯 O QUE É?

Política que estabelece **TOLERÂNCIA ZERO** para corrupção e suborno no {{COMPANY_NAME}}.

## ⚠️ O QUE É PROIBIDO?

❌ Oferecer/dar/prometer suborno ou propina  
❌ Solicitar/aceitar vantagens indevidas  
❌ Pagamentos de facilitação ("taxa"para agilizar serviço público)  
❌ Presentes/brindes acima dos limites estabelecidos  
❌ Contratar fornecedor sem due diligence

## ✅ REGRAS PARA BRINDES/PRESENTES

| Valor | Ação |
|-------|------|
| **Até R$ 100** | ✅ Aceitável (brindes promocionais) |
| **R$ 100 - R$ 500** | ⚠️ Notificar gestor antes de aceitar |
| **> R$ 500** | 🚫 Devolver ou doar |

## 👥 RELACIONAMENTO COM AGENTES PÚBLICOS

-  Reuniões com ≥2 pessoas do {{COMPANY_NAME}}
-  Documentar todos os contatos
-  Jamais oferecer pagamento ou vantagem indevida
- ⚠️ Hospitalidades (jantares, eventos) requerem aprovação prévia

## 📞 CANAIS

**Dúvidas:** {{COMPLIANCE_EMAIL}}  
**Denúncias:** {{WHISTLEBLOWING_EMAIL}} (anônimo permitido)

## 📋 DOCUMENTO COMPLETO

[Política de Anticorrupção](../../anti-corruption-policy.md)

---

**{{COMPANY_NAME}} © 2025 | ZERO TOLERÂNCIA**
